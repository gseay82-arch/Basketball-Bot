...
export default {
  data: new SlashCommandBuilder()
    .setName("demote")
    .setDescription("Demote your General Manager back to Player")
    .addUserOption(option =>
      option
        .setName("player")
        .setDescription("GM to demote")
        .setRequired(true)
    ),
  ...
  async execute(interaction) {
    ...
    if (!interaction.member.roles.cache.has(HEAD_COACH_ROLE_ID)) {
      return interaction.reply({
        content: "❌ You must have the Head Coach role to demote a GM.",
        ephemeral: true,
      });
    }
    ...
    if (!coachTeam) {
      return interaction.reply({
        content: "❌ You must have your team role to demote a GM.",
        ephemeral: true,
      });
    }
    ...
    if (!freshTarget.roles.cache.has(coachTeam.roleId)) {
      return interaction.reply({
        content: `❌ ${freshTarget} is not on **${coachTeam.name}**.`,
        ephemeral: true,
      });
    }

    if (!freshTarget.roles.cache.has(GENERAL_MANAGER_ROLE_ID)) {
      return interaction.reply({
        content: `❌ ${freshTarget} does not have the General Manager role.`,
        ephemeral: true,
      });
    }

    try {
      await freshTarget.roles.remove(GENERAL_MANAGER_ROLE_ID);
      await freshTarget.roles.add(PLAYER_ROLE_ID);
    } catch (error) {
      console.error(error);

      return interaction.reply({
        content: "❌ I could not update this member's roles. Check my role permissions.",
        ephemeral: true,
      });
    }
    ...
    if (transactionsChannel) {
      await transactionsChannel.send(
        `⬇️ **DEMOTION:** ${freshTarget} has been demoted from General Manager to Player for **${coachTeam.name}**.\nProcessed by: ${interaction.user}`
      );
    }

    return interaction.reply({
      content: `⬇️ ${freshTarget} has been demoted from General Manager to Player for **${coachTeam.name}**.`,
    });
  },
};
